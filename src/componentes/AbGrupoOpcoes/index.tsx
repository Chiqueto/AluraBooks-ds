...
export interface AbGrupoOpcoesProps {
    opcoes: AbGroupOpcao[],
    valorPadrao?: AbGroupOpcao | null,
    onChange?: (opcao: AbGroupOpcao) => void
}
...
export const AbGrupoOpcoes = ({ opcoes, onChange, valorPadrao }: AbGrupoOpcoesProps) => {
    const [selecao, setSelecao] = useState<AbGroupOpcao | null>(valorPadrao ?? null);

    const aoSelecionar = (opcao: AbGroupOpcao): void => {
        setSelecao(opcao);
        if (onChange) {
            onChange(opcao);
        }
